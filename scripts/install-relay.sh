#!/bin/bash
# RouteXia Relay Server — All-in-One Standalone Deploy Script
# Deploy to: Ubuntu 20.04 / 22.04 VPS (Singapore / India / Dubai)
# Usage: sudo ./install-relay.sh [SG|IN|DXB]

set -e

RELAY_PORT=9001
REGION=${1:-SG}
BINARY_NAME="routexia-relay"
WORK_DIR="/opt/routexia-relay"

echo "=== RouteXia Relay Server Setup (region: $REGION) ==="
...
# ── 2. Create source directory & write Go code ────────────────────────────────
echo "[+] Creating server source files in $WORK_DIR..."
mkdir -p "$WORK_DIR"
cd "$WORK_DIR"
...
# ── 4. Open UDP port in firewall ──────────────────────────────────────────────
echo "[+] Opening UDP port $RELAY_PORT in firewall..."
ufw allow $RELAY_PORT/udp 2>/dev/null || true
iptables -I INPUT -p udp --dport $RELAY_PORT -j ACCEPT 2>/dev/null || true

# ── 5. Create systemd service ─────────────────────────────────────────────────
echo "[+] Creating systemd service..."
cat > /etc/systemd/system/routexia-relay.service << EOF
[Unit]
Description=RouteXia Relay Server ($REGION)
After=network.target

[Service]
Type=simple
ExecStart=/usr/local/bin/$BINARY_NAME -port $RELAY_PORT -region $REGION
Restart=always
RestartSec=3
LimitNOFILE=65536
...
# Kill any lingering old process on the UDP port
fuser -k -9 $RELAY_PORT/udp 2>/dev/null || true
killall -9 $BINARY_NAME 2>/dev/null || true

systemctl daemon-reload
systemctl enable routexia-relay
systemctl restart routexia-relay
systemctl status routexia-relay --no-pager

echo ""
echo "=================================================="
echo "  ✅ RouteXia Relay Server Ready! ($REGION)"
echo "  Port   : UDP $RELAY_PORT"
echo "  Logs   : journalctl -u routexia-relay -f"
echo "=================================================="
